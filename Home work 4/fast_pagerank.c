...
void matmul_matrix(double *A, double *B, double *C, int N) {
    int i, j;
    #pragma omp parallel private(i, j) shared(A, B, C, N) default(none)
    {
        #pragma omp for
        for (i = 0; i < N; ++i) {
            C[i] = 0;
...
double l1_dist_matrix(double *A, double *B, int N) {
    double result = 0;
    int i;
    #pragma omp parallel private(i) shared(result, A, B, N) default(none)
    {
...
void pagerank_matrix(double *matrix, double *result, int N, double eps, double damping, int max_iter) {
    ...
    int i, j;
    #pragma omp parallel private(i, j) shared(matrix, f_matrix, N) default(none)
    {
        #pragma omp for
        for (i = 0; i < N; ++i) {
            double row_sum = 0;            
...
void naive_ranking(double *matrix, double *ranking, int N) {
    int i, j;
    double sum_rank = 0;
    #pragma omp parallel private(i, j) shared(sum_rank,ranking, matrix, N) default(none)
    {
        #pragma omp for reduction(+:sum_rank)
        for (i = 0; i < N; ++i) {
            ranking[i] = 0;
            for (j = 0; j < N; ++j) {
